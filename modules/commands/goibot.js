...
module.exports.handleEvent = function({ api, event }) {
  ...
  var tl = ["Sử dụng callad để liên lạc với admin", "Đừng spam em nha :<<, cậu chủ em mệt lắm ời","Yêu em 💗,đừng chửi bot hay spam bot nha bot bùn lắm ớ 🥺"];
  var rand = tl[Math.floor(Math.random() * tl.length)]

  if ((event.body.toLowerCase() == "bot ngu")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua Fb chính để được hỗ trợ\nLink fb nè: https://www.facebook.com/Tungthin.19122003", threadID, () =>
  ...
  if ((event.body.toLowerCase() == "Tùng") || (event.body.toLowerCase() == "Tung")) {
  ...
  if ((event.body.toLowerCase() == "Đặng Viết Tùng") || (event.body.toLowerCase() == "Đặng Viết Tùng")) {
    return api.sendMessage("Anh ấy tuy ko đẹp trai hay giỏi gì nhưng được cái hài hước và luôn mong đạt được mục tiêu anh ấy muốn <3 <3 ", threadID);
  };

  if ((event.body.toLowerCase() == "bot ơi") ||  (event.body.toLowerCase() == "bot ơi")) {
    return api.sendMessage("Nói đii lẹ, bot còn phải phục vụ các box khác nữa :3", threadID);
  };

  if ((event.body.toLowerCase() == " ibot") || (event.body.toLowerCase() == "yeu bot")) {
    return api.sendMessage("Hmm... Bot ngại lắm thôi yêu admin của bot nhá :>", threadID);
  };
 ...
 if ((event.body.toLowerCase() == "bsvv nha mng") || (event.body.toLowerCase() == "bsvv nha mng")) {
  ...
  if ((event.body.toLowerCase() == "admin m là ai v bot") || (event.body.toLowerCase() == "admin m la ai v bot")) {
    return api.sendMessage("️Admin tao Đặng Viết Tùng có gì dùng lệnh !ad hoặc !adm để biết thêm chi tiết", threadID);
  };

  if ((event.body.toLowerCase() == "admin m là ai vậy bot") || (event.body.toLowerCase() == "admin m la ai vậy bot")) {
    return api.sendMessage("️Admin tao Dang Viet Tung  có gì dùng lệnh .ad hoặc .adm để biết thêm chi tiết", threadID);
  };
